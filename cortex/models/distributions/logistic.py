...
def _neg_logistic_log_prob(x, p, clip=None, sum_probs=True):
    ...
    if clip is not None: log_s = T.maximum(log_s, clip)
    g = (x - mu) / T.exp(log_s)
...
def _logistic_unit_variance(trng, mu, size=None):
    if size is None: size = mu.shape
    epsilon = trng.uniform(size=size, dtype=floatX)
    return mu + T.log(epsilon / (1 - epsilon))

def _neg_logistic_log_prob_unit_variance(x, mu, clip=None, sum_probs=True):
    g = (x - mu)
    energy = -g + 2 * T.log(1 + T.exp(g))
    if sum_probs:
        return energy.sum(axis=energy.ndim-1)
    else:
        return energy

def _logistic_entropy_unit_variance(mu, clip=None):
    entropy = 2.0
    return entropy.sum(axis=entropy.ndim-1)
...
class Logistic(Distribution):
    ...
    def __init__(self, dim, name='logistic', clip=None, **kwargs):
...
class LogisticUnitVariance(Distribution):
    '''Logistic distribution with unit variance.
    ...
    def __init__(self, dim, name='logistic', clip=None, **kwargs):
        self.f_sample = _logistic_unit_variance
        self.f_neg_log_prob = _neg_logistic_log_prob_unit_variance
        self.f_entropy = _logistic_entropy_unit_variance
        self.clip = clip
        super(LogisticUnitVariance, self).__init__(dim, name=name, **kwargs)
    ...
    def permute(self, scale=2.):
        h0 = self.mu
        s = T.nlinalg.AllocDiag()(T.ones_like(h0)).astype(floatX)
        h = scale * s + h0[None, :]
        return OrderedDict(mean=h0, perm=h)
...
_classes = {'logistic': Logistic, 'logistic_unit_variance': LogisticUnitVariance}